...
export default {
  ...
  mounted() {
    ...
    ctx.canvas.width = canvas.clientWidth; //window.innerWidth;
    ctx.canvas.height = canvas.clientHeight; //window.innerHeight;
    ...
    class Vect {
      ...
      get realx() {
        return Math.floor((winPos.x + this.x) / CELL_SIZE);
      }
      get realy() {
        return Math.floor((winPos.y + this.y) / CELL_SIZE);
      }
